...
def goodbyecruelworld():
    ...
    if origin not in allowed_origins and (not referer or not any(referer.startswith(o) for o in allowed_origins)):
        return """
<h1>EEK!</h1>
...
def executeaccountdeletion():
    ...
    try:
        db = get_db()
        db.execute("DELETE FROM tasks WHERE ownerID = ?", (session["github_id"],))
        db.execute("DELETE FROM users WHERE userID = ?", (session["github_id"],))
        db.commit()

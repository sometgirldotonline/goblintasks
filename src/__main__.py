...
from flask import Flask, redirect, url_for, render_template, g, make_response, request, jsonify, session
import jinja2
import os
import sqlite3
import json
...
""",(60, session["github_id"]))
    ...
    cur = db.execute("SELECT COALESCE(SUM(taskValue), 0) AS total_value FROM tasks WHERE ownerID = ? and taskCompletion = 1;", (session["github_id"],))
    res = cur.fetchone()
    username = session["github_login"]
    cur = db.execute("SELECT * FROM tasks WHERE ownerID = ?", (session["github_id"],))
    return render_template("app.html", username=username, usercoins=res['total_value'] + usercoins, tasks=cur.fetchall())
...
def uts():
    ...
    if 'id' in request.form and 'state' in request.form:
        ...
        if not github.authorized:
            return redirect(url_for("github.login"))
        db = get_db()
        cur = db.execute("SELECT * FROM tasks WHERE ownerID = ? and taskID = ?", (session["github_id"],request.form['id'],))
        res = cur.fetchall()
        if len(res) == 1:
            # Debug: Show what we're about to update
            new_completion_value = 1 if request.form["state"] == "1" else 0
            print(f"DEBUG: Updating taskID {request.form['id']} to completion {new_completion_value}")
            ...
            cur = db.execute("SELECT taskCompletion FROM tasks WHERE ownerID = ? and taskID = ?", (session["github_id"],request.form['id'],))
            updated_result = cur.fetchall()
            print(f"DEBUG: After update, taskCompletion = {updated_result[0]['taskCompletion'] if updated_result else 'NOT FOUND'}")
